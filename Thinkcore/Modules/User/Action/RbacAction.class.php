    ...
    public function listorders() {
       if(IS_POST){
            ...
            foreach($listorders as $roleid => $listorder) {
                if($this->Role->where(array('id'=>$roleid))->save(array('listorder'=>$listorder))){
                    $str.=$roleid.":".$listorder.",";   
                }
            }
            $msg = "更新排序成功！(".$str."）";
            //$this->show($msg)  ;
    ...
    public function authorize() {
        ...
        if (IS_POST) {
            ...
            $str = "<tr id='node-\$id' \$parentid_node>
                           <td style='padding-left:15px;'>\$spacer<input type='checkbox' name='menuid[]' value='\$id' level='\$level' \$checked onclick='javascript:checknode(this);'> \$name\$tip</td>
	    </tr>";
            $menu->init($result);
