...
class AttachmentService {
    ...
    public function water($source, $water = null, $savename = null, $alpha = null, $waterPos = null, $quality = null) {
        ...
        //设置默认水印
        if ($water == '') {
            $water = SITE_PATH . $this->config['CFG_WATERMARK_PIC'];
        }
        //图像信息
        $sInfo = Image::getImageInfo($source);
        //如果图片小于系统设置，不进行水印添加
        if ($sInfo["width"] < (int) $this->config['CFG_WATERMARK_MINWIDTH'] || $sInfo['height'] < (int) $this->config['CFG_WATERMARK_MINHEIGHT']) {
            return false;
        }
        //水印位置
        if (empty($waterPos)) {
            $waterPos = (int) $this->config['CFG_WATERMARK_POSITION'];
        }
        //水印透明度
        if (empty($alpha)) {
            $alpha = (int) $this->config['CFG_WATERMARK_PCT'];
        }
        //jpg图片质量
        if (empty($quality)) {
            $quality = (int) $this->config['CFG_WATERMARK_QUALITY'];
        }
