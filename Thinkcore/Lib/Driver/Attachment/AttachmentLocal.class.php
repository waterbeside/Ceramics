...
class AttachmentLocal extends AttachmentService {
    ...
    function __construct($options = array()) {
        ...
        $options = array_merge(array(
            ...
            'module' => 'contents ',
            //是否添加水印
            'watermarkenable' => $this->config['CFG_WATERMARK_TYPE'],
            //生成缩略图
            'thumb' => false,
            //上传时间戳
            'time' => time(),
        ...
        //附件访问地址 
        $this->options['sitefileurl'] = $this->config['CFG_FILE_URL'];
        //附件存放路径
        $this->options['uploadfilepath'] = C('UPLOADFILEPATH');
        //允许上传的附件大小
        if (empty($this->options['uploadmaxsize'])) {
            $this->options['uploadmaxsize'] = $this->options['isadmin'] ? (int) $this->config['CFG_UPLOAD_MAXSIZE'] * 1024 : (int) $this->config['CFG_UPLOAD_FRONT_MAXSIZE'] * 1024;
        }
        //允许上传的附件类型
        if (empty($this->options['uploadallowext'])) {
            $this->options['uploadallowext'] = $this->options['isadmin'] ? explode("|", $this->config['CFG_UPLOAD_ALLOW_EXT']) : explode("|", $this->config['CFG_UPLOAD_FRONT_ALLOW_EXT']);
        }
        //上传目录
