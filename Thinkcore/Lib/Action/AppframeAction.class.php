   ...
   /* protected function ajaxReturn($data, $type = '') {
    ...
    final protected function initSite() {
        ...
        if (C("APP_SUB_DOMAIN")) {
            ...
            $config_siteurl = $Config['CFG_SITE_URL'];
            define("CONFIG_SITEURL_MODEL", $config_siteurl);
            $this->assign("config_siteurl", $config_siteurl);
        }
        //去除敏感信息
        unset($Config['CFG_MAIL_SMTP_PASS']);
     ...
     */
    final public function addLogs($info, $status = 1) {
        D('Log')->record($info, $status);
    }
    ...
    static public function verify($verify, $type = "verify") {
        ...
        if (isset($_SESSION[$type]) && $_SESSION[$type] == md5(strtolower($verify))) {
            $_SESSION[$type] = null;
            return true;
        } else {
            return false;
