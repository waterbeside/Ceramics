...
class Image {
     ...
     * @return string
      +----------------------------------------------------------
     */
    static function buildImageVerify($length = 4, $mode = 1, $type = 'png', $width = 48, $height = 22, $verifyName = 'verify', $size='0') {
        ...
        $fontPath=LIB_PATH."datas/ttfs/4.ttf"; 

        $size = $size > 0 ? $size : $height/1.6-10/1.6;
        
        for ($i = 0; $i < $length; $i++) {
            //imagestring($im, 5, $i * 10 + 5, mt_rand(1, 8), $randval{$i}, $stringColor);
            imagettftext($im, $size, mt_rand(-30,30) , $i * 16 + 15, $height/1.4, $stringColor, $fontPath, $randval{$i}); //这个是新的，用imagettftext函数
        }
    ...
    static function GBVerify($length = 4, $type = 'png', $width = 180, $height = 50, $fontface = 'msyh.ttf', $verifyName = 'verify') {
        ...
        if (!is_file($fontface)) {
            $fontPath = dirname(__FILE__) . "/" . $fontface;
        }
         if (!is_file($fontface)) {
            $fontPath=LIB_PATH."datas/ttfs/" . $fontface;          
        }
       
        for ($i = 0; $i < $length; $i++) {
            $fontcolor = imagecolorallocate($im, mt_rand(0, 120), mt_rand(0, 120), mt_rand(0, 120)); //这样保证随机出来的颜色较深。
            $codex = String::msubstr($code, $i, 1); 
            imagettftext($im, mt_rand(16, 20), mt_rand(-60, 60), 40 * $i + 20, mt_rand(30, 35), $fontcolor, $fontPath, $codex);
        }
        Image::output($im, $type);
    }
