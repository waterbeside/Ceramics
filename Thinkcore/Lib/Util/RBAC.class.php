...
class RBAC {
    ...
    // 认证方法
    static public function checkMapAccess($map) {
        ...
        //使用给定的Map进行认证
        $map_default = array('g' => strtoupper(GROUP_NAME), 'm'=>strtoupper(MODULE_NAME),'a'=> strtoupper(ACTION_NAME));

        if(is_array($map)){
            $appName = isset($map['g']) ? strtoupper($map['g']) :$map_default['g'];
            $module = isset($map['m']) ? strtoupper($map['m']) :$map_default['m'];
            $action = isset($map['a']) ? $map['a'] :$map_default['a'];
        }else{
            $map = trim($map, '/');
            if(strpos('/', $map)>0){
                $map = explode('/', $map);
                ...
                switch ($cm) {
                    case 1:
                        $appName =  $map_default['g'];
                        $module  =  $map_default['m'];
                        $action  =  strtoupper($map[0]);
                        break;
                    case 2:
                        $appName =  $map_default['g'];
                        $module  =  $map[0];
            ...
            if (empty($ADMIN_AUTH_KEY)) {
                ...
                if (!isset($accessList[strtoupper($appName)][strtoupper($module)][strtoupper(ACTION_NAME)])) {
                    ...
                    if (self::checkLogin() == true) {
                        ...
                        if (($appName == "Oa"||$appName == "Admin") && in_array(MODULE_NAME, array("Index", "Main")) && in_array(ACTION_NAME, array("index"))) {
