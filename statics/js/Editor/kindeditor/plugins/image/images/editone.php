...
$CONFIG=array("version"=>"V1.4RC/070516", "nLayer"=>"1", "auth_type"=>"cookie", "cookie_life"=>"43200", "session_life"=>"20", "realm"=>"EditONE@{server_name}", "start_dir"=>".", "base_dir"=>null, "encoding"=>"GB2312", "encoding_list"=>"简体中文|GB2312|936||统一码|UTF-8|65001||繁体中文|BIG5|950||latin1|ISO8859-1|1252||日语|shift_jis|932||韩文|ks_c_5601|949", "wrap_word"=>"undefined", "editor_font_family"=>"宋体", "editor_font_height"=>"14", "editor_line_height"=>"16", "fckeditor_dir"=>"fckeditor/", "list_sort"=>"name");//CONFIG
	...
	elseif ($CONFIG["auth_type"]=="HTTP") {
		...
		elseif ($fileType=="HTML")
			if (isset($_SERVER["PHP_AUTH_USER"]) && $AUTH_USER["password"]==md5($_SERVER['PHP_AUTH_PW']) && $AUTH_USER["username"]==$_SERVER["PHP_AUTH_USER"]) {
				...
				print "抱歉，您的密码不正确，或者您没有使用此软件的权限。请联系<a href='mailto:".$AUTH_USER["email"]."'>".$AUTH_USER["email"]."</a>";
				exit();
			}
...
<table border='0' width='95%' id='table1'>
	<tr>
		<td width='100'>　</td>
		<td><font color='#FFC080' style='font-size:14px; font-family:arial black'>EditONE</font>  更快捷、更方便、更简单<br>{$CONFIG['version']} <a class='gray' href='http://asp99.cn/' target='_blank'>下载最新版本</a>&nbsp; <a class='gray' href='http://asp99.cn' target='_blank'>建议或Bug报告</a>&nbsp; <a class='gray' href='http://asp99.cn' target='_blank'>全能空间</a><font color='#999999' style='font-size:12px'>，版权保留。</font></td>
	</tr>
	<tr>
		<td>　</td>
		<td>　</td>
...
<a class="gray" href="?do=config" title="映射物理路径：{$AbsPath}" target="mainFrame">配置</a>&nbsp;
...
function showMainFrame()
{
	...
	if($CONFIG["fckeditor_dir"]===null)
		$fckeditor="disabled title='尚未安装可视化编辑器FCKeditor。请到http://www.fckeditor.net/下载最新版本上传到网站空间里，然后设置CONFIG['fckeditor_dir']为FCKeditor安装目录，如'fckeditor/'，即可使用可视化编辑器'";
	else
		$fckeditor="title='适合于静态HTML文件“所见即所得”模式的可视化编辑，但一般会破坏服务器端脚本代码的内容。请勿勾选“共享编辑”选项直接编辑ASP/PHP/JSP等程序文件。'";
	print <<<EOF
$HTML_HEADER
<style>
...
<form name="M" method="POST" action="?">
	...
	<tr>
		...
		<td>编码：<select name="encoding"><option value="auto">自动</option><option value="{$CONFIG["encoding"]}">ANSI ({$CONFIG["encoding"]})</option>
			...
			<input {$fckeditor} type="radio" class="box" name="mode" value="html" onClick="switchMode(this.value)">可视化
...
function showEditFile()
{
	...
	if (!($fp=fopen($AbsPath, "rb"))) {
		$reason=str_replace("&", "&amp;", "无法读取文件“ ".$AbsPath." ”。");
...
function responseSaveFile()
{
	...
	if (!(is_writable($AbsPath) && ($fp=fopen($AbsPath, "wb")))) {
		$reason=str_replace("&", "&amp;", "无法写文件“{$AbsPath}”。");
		$advice="请检查文件是否被锁定以及所在目录的读写性。";
		print <<<EOF
<File>
<Status>false</Status>
...
function newFile()
{
	...
	if ($status) {
		$reason=str_replace("&", "&amp;", "无法创建".$TypeName."“ ".$name." ”。目标".$TypeName."名已存在。");
		$advice=str_replace("&", "&amp;", "请更名为其他".$TypeName."名或将".$TypeName."“ ".$name." ”更名。");
		print <<<EOF
<File>
<Status>false</Status>
<Reason>{$reason}</Reason>
...
EOF;
		...
		$reason=str_replace("&", "&amp;", "无法创建".$TypeName."“ ".$name." ”。");
...
EOF;
		...
		$reason=str_replace("&", "&amp;", "无法创建".$TypeName."“ ".$destfile." ”。");
		$advice="请检查".$TypeName."是否被锁定以及所在目录的读写性。";
		print <<<EOF
<File>
<Status>false</Status>
...
function downloadFile()
{
	...
	print "<html><head><meta content='text/html; charset={$CONFIG['encoding']}' http-equiv='Content-Type'></head><body><script>alert('下载文件失败。\\n\\n原因：\\n无法打开文件“ ".$name." ”。\\n\\n建议：\\n请检查文件是否存在，并有读取权限。')</script></body></html>";
}
...
EOF;
		...
		if ($status) {
			$reason=str_replace("&", "&amp;", "无法重命名文件“ ".$file0." ”为“ ".$name." ”。目标文件名已存在。");
			$advice=str_replace("&", "&amp;", "请更名为其他文件名或将文件“ ".$name." ”更名。");
		}
		else {
			$reason=str_replace("&", "&amp;", "无法重命名文件“ ".$file0." ”为“ ".$name." ”。");
			$advice="请检查文件是否被锁定以及所在目录的读写性。";
		}
		print <<<EOF
<File>
...
EOF;
		...
		$reason=str_replace("&", "&amp;", "无法删除文件“ ".$file0." ”。");
		$advice="请检查文件是否被锁定以及所在目录的读写性。";
		print <<<EOF
<File>
<Status>false</Status>
...
EOF;
		...
		print<<<EOF
<html><head><meta content='text/html; charset={$CONFIG['encoding']}' http-equiv='Content-Type'></head><body><script>parent.T.uploadReport(false,'无法写文件“{$filename}”。','请检查文件是否被锁定以及所在目录的读写性。');</script></body></html>
EOF;
}
...
<form method="POST" action="?do=modify" name="F" onSubmit="return check()">
	...
	<script>document.F.auth_type.value="{$CONFIG['auth_type']}";</script>
		...
		<tr>
			...
			<td height="20"><input type="text" name="base_dir" size="20" value="{$base_dir}"> 必须是一个绝对路径，仅当起始目录是以'/'开头时有效。留空表示映射到网站的根目录。</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td height="20"><input type="text" name="fckeditor_dir" size="20" value="{$fckeditor_dir}"> FCKediotor的安装目录，注意末尾带上'/'。</td>
		</tr>
		<tr>
